...
const Season = () => {
  ...
  const getSeasonInfo=async()=>
  {
    try{
        ...
        arrayData.push(data)
        const chosenData=(arrayData[0].data[arrayData.length-1]); 
  ...
  return (
    <section className="flex relative w-full h-100 justify-center items-center ">
        <Image src={`https://pbs.twimg.com/media/GJcn6vUXUAEwtsZ.jpg:large`}  width={2000} height={2000} className="w-full block
        " alt=""/>
            {season &&
          
                    <div className=" absolute flex items-center justify-center">
                        <div className="bg-[#DCD9D4] px-4 py-4 rounded">
                            <h2 className="text-[#ff4656] font-bold text-3xl md:text-5xl">{season.displayName}</h2>
                        </div>
                    </div>
